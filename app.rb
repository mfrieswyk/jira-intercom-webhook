  ...
  if ['jira:issue_created', 'jira:issue_updated'].include?(json['webhookEvent'])
    ...
    match_data.each do |data1|
      data1.each do |data2|
        ...
        result = INTERCOM_CLIENT.note_conversation(convo_id, "<a href='#{issue_url}' target='_blank'>#{issue_type} [#{issue_key}] #{issue_title} </a></br><b>Status:</b> #{issue_status} <b>Assigned to:</b> #{assignee}")
        result.to_json
      end
    end
  end
